...
const Browse = () => {
	...
	for (let i = 1; i <= Math.floor(totalMovies / numberOfMoviesPerButton); i++) {
		buttonPlacesData.push({
			number: i,
			numberOfMovies: numberOfMoviesPerButton,
		});
	...
	if (remainingMovies > 0) {
		buttonPlacesData.push({
			number: buttonPlacesData.length + 1,
			numberOfMovies: remainingMovies,
		});
	}
	...
	return (
		<div className="flex-container">
			<div className="DMFilter">
				...
				<button className="DMSearch" onClick={checkSelections}>
					Search
				</button>
			</div>
